...
public abstract class HoodieClientTestHarness extends HoodieCommonTestHarness implements Serializable {
  ...
  protected transient HoodieSparkReadClient readClient;
  protected transient HoodieTableFileSystemView tableView;
  protected transient HoodieSparkTable hoodieTable;
  ...
  public void cleanupResources() throws IOException {
    ...
    cleanupHoodieSparkEngineContext();
  ...
  /**
   * Initializes  HoodieSparkEngineContext with {@link JavaSparkContext}.
   */
  protected void initHoodieSparkEngineContext() {
  ...
  /**
   * Cleanups Spark contexts ({@link HoodieSparkEngineContext}.
   */
  protected void cleanupHoodieSparkEngineContext() {
    if (context != null) {
      LOG.info("Closing HoodieSparkEngineContext used in previous test-case");
      context = null;
    }
  }
  ...
  public HoodieSparkWriteClient getHoodieWriteClient(HoodieWriteConfig cfg, boolean rollbackInflightCommit) {
    return getHoodieWriteClient(cfg, rollbackInflightCommit, HoodieSparkIndexFactory.createIndex(cfg));
  }

  public HoodieSparkReadClient getHoodieReadClient(String basePath) {
    HoodieWriteConfig writeConfig = HoodieWriteConfig.newBuilder().withPath(basePath)
        .withIndexConfig(HoodieIndexConfig.newBuilder().withIndexType(HoodieIndex.IndexType.BLOOM).build()).build();
    readClient = new HoodieSparkReadClient(new HoodieSparkEngineContext(jsc), writeConfig);
    return readClient;
  }
  ...
  public HoodieSparkWriteClient getHoodieWriteClient(HoodieWriteConfig cfg, boolean rollbackInflightCommit,
    ...
    writeClient = new HoodieSparkWriteClient(new HoodieSparkEngineContext(jsc), cfg, rollbackInflightCommit, HoodieSparkTable.create(metaClient, cfg, jsc.hadoopConfiguration()));
    return writeClient;
  }
