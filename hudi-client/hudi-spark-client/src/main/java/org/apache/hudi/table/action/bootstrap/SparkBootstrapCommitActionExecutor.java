...
import org.apache.hudi.client.bootstrap.BaseBootstrapSchemaProvider;
...
import org.apache.hudi.io.HoodieSparkBootstrapHandle;
...
public class SparkBootstrapCommitActionExecutor<T extends HoodieRecordPayload>
  ...
  private void updateIndexAndCommitIfNeeded(JavaRDD<WriteStatus> writeStatusRDD, HoodieWriteMetadata result) {
  ...
  private BootstrapWriteStatus handleMetadataBootstrap(String srcPartitionPath, String partitionPath,
    ...
    HoodieSparkBootstrapHandle bootstrapHandle = new HoodieSparkBootstrapHandle(config, HoodieTimeline.METADATA_BOOTSTRAP_INSTANT_TS,
        table, partitionPath, FSUtils.createNewFileIdPfx(), (SparkTaskContextSupplier) table.getTaskContextSupplier());
  ...
  private Map<BootstrapMode, List<Pair<String, List<HoodieFileStatus>>>> listAndProcessSourcePartitions() throws IOException {
    ...
    BaseBootstrapSchemaProvider sourceSchemaProvider = new HoodieSparkBootstrapSchemaProvider(config);
