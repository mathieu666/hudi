...
public abstract class HoodieTable<T extends HoodieRecordPayload, INPUT extends HoodieWriteInput, KEY extends HoodieWriteKey, OUTPUT extends HoodieWriteOutput> implements Serializable {
  ...
  private static SerializableConfiguration hadoopConf;
  private transient FileSystemViewManager viewManager;

  protected final SparkTaskContextSupplier sparkTaskContextSupplier = new SparkTaskContextSupplier();

  protected HoodieTable(HoodieWriteConfig config, Configuration configuration, HoodieTableMetaClient metaClient) {
    this.config = config;
    hadoopConf = new SerializableConfiguration(configuration);
    this.viewManager = FileSystemViewManager.createViewManager(new SerializableConfiguration(configuration),
        config.getViewStorageConfig());
    this.metaClient = metaClient;
    this.index = new HBaseIndexV2(configuration, config);
  }

  private synchronized FileSystemViewManager getViewManager() {
    if (null == viewManager) {
      viewManager = FileSystemViewManager.createViewManager(hadoopConf, config.getViewStorageConfig());
    }
    return viewManager;
  }
  ...
  public static <T extends HoodieRecordPayload,
    ...
    switch (metaClient.getTableType()) {
      case COPY_ON_WRITE:
        return TableFactory.createCopyOnWriteTable(config, hadoopConf.get(), metaClient);
      case MERGE_ON_READ:
        return TableFactory.createMergeOnReadTable(config, hadoopConf.get(),metaClient);
      default:
