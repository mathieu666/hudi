...
public abstract class HoodieTable<T extends HoodieRecordPayload, INPUT extends HoodieWriteInput, KEY extends HoodieWriteKey, OUTPUT extends HoodieWriteOutput> implements Serializable {
  ...
  protected HoodieTable(HoodieWriteConfig config, HoodieTableMetaClient metaClient, HoodieEngineContext context) {
    ...
    this.viewManager = FileSystemViewManager.createViewManager(context.getHadoopConfiguration(),
        config.getViewStorageConfig());
    this.metaClient = metaClient;
    this.index = new HoodieIndexV2(config, context);
  }

  private synchronized FileSystemViewManager getViewManager() {
    if (null == viewManager) {
      viewManager = FileSystemViewManager.createViewManager(context.getHadoopConfiguration(), config.getViewStorageConfig());
    }
    return viewManager;
  }
   ...
   * @return
   */
  public abstract HoodieCompactionPlan scheduleCompaction(HoodieEngineContext context, String instantTime);
