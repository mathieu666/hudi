...
public class UpsertPartitioner<T extends HoodieRecordPayload<T>> implements Partitioner {
  ...
  public UpsertPartitioner(WorkloadProfile profile, Configuration hadoopConf, HoodieTable<T> table,
    ...
    assignInserts(profile, hadoopConf);
  ...
  private void assignInserts(WorkloadProfile profile, Configuration hadoopConf) {
    ...
    Map<String, List<SmallFile>> partitionSmallFilesMap =
        getSmallFilesForPartitions(new ArrayList<String>(partitionPaths), hadoopConf);
  ...
  private Map<String, List<SmallFile>> getSmallFilesForPartitions(List<String> partitionPaths, Configuration hadoopConf) {

    Map<String, List<SmallFile>> partitionSmallFilesMap = new HashMap<>();
    if (partitionPaths != null && partitionPaths.size() > 0) {
      partitionSmallFilesMap = partitionPaths.stream().map(
          partitionPath -> Pair.of(partitionPath, getSmallFiles(partitionPath))).collect(Collectors.toMap(Pair::getLeft, Pair::getRight));
    }
