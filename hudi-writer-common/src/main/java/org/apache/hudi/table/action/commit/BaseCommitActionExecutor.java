...
public abstract class BaseCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  public BaseCommitActionExecutor(HoodieEngineContext jsc, HoodieWriteConfig config,
  ...
  public HoodieWriteMetadata execute(HoodieWriteInput inputRecordsRDD) {
...
//      profile = context.buildProfile(inputRecordsRDD);
//      LOG.info("Workload profile :" + profile);
//      saveWorkloadProfileMetadataToInflight(profile, instantTime);
//    }

    // partition using the insert partitioner
    HoodieWriteOutput writeStatusRDD = context.lazyWrite(inputRecordsRDD, instantTime);
    // here we just update the index, ignore commit(or make sure commit is disabled).
  ...
  private HoodieWriteInput partition(HoodieWriteInput dedupedRecords, Partitioner partitioner) {
  ...
  protected void updateIndexAndCommitIfNeeded(HoodieWriteOutput writeStatus, HoodieWriteMetadata result) {
    Instant indexStartTime = Instant.now();
    // Update the index back
    context.updateLocation(writeStatus,table,context);
    result.setIndexUpdateDuration(Duration.between(indexStartTime, Instant.now()));
    result.setWriteStatuses(writeStatus);
