...
public abstract class BaseCommitActionExecutor<T extends HoodieRecordPayload<T>, I, K, O, P>
  ...
  public BaseCommitActionExecutor(HoodieEngineContext context, HoodieWriteConfig config,
                                  HoodieTable<T, I, K, O, P> table, String instantTime, WriteOperationType operationType, TaskContextSupplier taskContextSupplier) {
    this(context, config, table, instantTime, operationType, taskContextSupplier, null);
  }

  public BaseCommitActionExecutor(HoodieEngineContext context, HoodieWriteConfig config,
                                  HoodieTable<T, I, K, O, P> table, String instantTime, WriteOperationType operationType, TaskContextSupplier taskContextSupplier,
                                  HoodieWriteInput<I> inputRecordsRDD) {
    super(context, config, table, instantTime);
    this.taskContextSupplier = taskContextSupplier;
    this.operationType = operationType;
  }

  public abstract HoodieWriteMetadata<O> execute(HoodieWriteInput<I> inputRecordsRDD);
  ...
  protected abstract void updateIndexAndCommitIfNeeded(HoodieWriteOutput<O> writeStatusRDD, HoodieWriteMetadata<O> result);
