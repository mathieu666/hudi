...
public class JsonKafkaSource extends JsonSource {
  ...
  private JavaRDD<String> toRDD(OffsetRange[] offsetRanges) {
    final JavaRDD<ConsumerRecord<Object, Object>> consumerRecordJavaRDD =
        KafkaUtils.createRDD(sparkContext, offsetGen.getKafkaParams(), offsetRanges,
            LocationStrategies.PreferConsistent());

    return transform(consumerRecordJavaRDD);
  }

  protected JavaRDD<String> transform(JavaRDD<ConsumerRecord<Object, Object>> consumerRecordJavaRDD) {
    return consumerRecordJavaRDD.filter(x -> Objects.nonNull(x.value())).map(x -> (String) x.value());
  }
