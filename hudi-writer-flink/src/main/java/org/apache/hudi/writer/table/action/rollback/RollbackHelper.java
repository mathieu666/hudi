...
public class RollbackHelper implements Serializable {
   ...
   */
  public List<HoodieRollbackStat> performRollback(Configuration hadoopConf, HoodieInstant instantToRollback, List<RollbackRequest> rollbackRequests) {
    ...
    Map<String, List<Pair<String, HoodieRollbackStat>>> keyedRoolbackStat = rollbackRequests.stream().map(rollbackRequest -> {
      ...
      switch (rollbackRequest.getRollbackAction()) {
        case DELETE_DATA_FILES_ONLY: {
          try {
            ...
            LOG.error("Delete CleanedFiles failed ", e);
          }
        ...
        case APPEND_ROLLBACK_BLOCK: {
          ...
          try {
            ...
            LOG.error("Get fileStatus failed ", e);
          }
          return Pair.of(rollbackRequest.getPartitionPath(),
              HoodieRollbackStat.newBuilder().withPartitionPath(rollbackRequest.getPartitionPath())
    ...
    return keyedRoolbackStat.values()
        .stream()
        .map(pairs -> pairs.stream().map(Pair::getRight).reduce(this::mergeRollbackStat).orElse(null))
