...
public abstract class AbstractHoodieWriteClient<T extends HoodieRecordPayload> extends AbstractHoodieClient {
   ...
   */
  public boolean commit(String instantTime, HoodieWriteOutput<List<WriteStatus>> writeStatuses) {
  ...
  protected HoodieWriteOutput<List<WriteStatus>> updateIndexAndCommitIfNeeded(HoodieWriteOutput<List<WriteStatus>> writeStatusRDD, HoodieTable<T> table,
                                                           String instantTime) {
    // Update the index back
    HoodieWriteOutput<List<WriteStatus>> statuses = index.updateLocation(writeStatusRDD, hadoopConf, table);
    // Trigger the insert and collect statuses
    commitOnAutoCommit(instantTime, statuses, table.getMetaClient().getCommitActionType());
    return statuses;
  }

  protected void commitOnAutoCommit(String instantTime, HoodieWriteOutput<List<WriteStatus>> resultRDD, String actionType) {
  ...
  private boolean commit(String instantTime, HoodieWriteOutput<List<WriteStatus>> writeStatuses,
    ...
    List<HoodieWriteStat> stats = writeStatuses.getOutput().stream().map(WriteStatus::getStat).collect(Collectors.toList());
  ...
  public void updateLocation(HoodieWriteOutput<List<WriteStatus>> writeResults, Configuration hadoopConf) {
    HoodieTableMetaClient metaClient = createMetaClient(true);
    HoodieTable table = HoodieTable.create(metaClient, config, hadoopConf);
