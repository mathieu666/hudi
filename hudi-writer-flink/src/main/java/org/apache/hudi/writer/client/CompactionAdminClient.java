...
import org.apache.hudi.writer.table.compact.OperationResult;
...
public class CompactionAdminClient extends AbstractHoodieClient {
  ...
  public CompactionAdminClient(Configuration hadoopConf, String basePath) {
    super(hadoopConf, HoodieWriteConfig.newBuilder().withPath(basePath).build());
  }
  ...
  private List<RenameOpResult> runRenamingOps(HoodieTableMetaClient metaClient,
    ...
    if (renameActions.isEmpty()) {
      ...
      if (!dryRun) {
        return renameActions.stream().map(lfPair -> {
          try {
