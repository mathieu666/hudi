...
public abstract class HoodieTable<T extends HoodieRecordPayload> implements Serializable {
  ...
  protected final HoodieHBaseIndex index;

  private SerializableConfiguration hadoopConfiguration;
  private transient FileSystemViewManager viewManager;

  protected HoodieTable(HoodieWriteConfig config, Configuration hadoopConf, HoodieHBaseIndex index) {
    this.config = config;
  ...
  public static <T extends HoodieRecordPayload> HoodieTable<T> getHoodieTable(HoodieTableMetaClient metaClient,
                                                                              HoodieWriteConfig config, Configuration hadoopConf,HoodieHBaseIndex index) {
    switch (metaClient.getTableType()) {
      case MERGE_ON_READ:
        return new HoodieMergeOnReadTable(config, hadoopConf,index);
      //      case COPY_ON_WRITE:
  ...
  public abstract Iterator<List<WriteStatus>> handleUpsertPartition(String instantTime, Integer partition,
                                                                    Iterator<HoodieRecord> recordIterator, PartitionHelper partitioner);
   ...
   */
  public abstract List<WriteStatus> compact(String compactionInstantTime,
                                            HoodieCompactionPlan compactionPlan);

  /**
   * Return the index.
   */
  public HoodieHBaseIndex getIndex() {
    return index;
  }
  ...
  protected void cleanFailedWrites(String instantTs, List<HoodieWriteStat> stats,
    ...
    try {
      ...
      if (!groupByPartition.isEmpty()) {
        ...
        groupByPartition.values().stream().map(partitionWithFileList -> {
  ...
  private void waitForAllFiles(Map<String, List<Pair<String, String>>> groupByPartition,
    ...
    // This will either ensure all files to be deleted are present.
    boolean checkPassed = groupByPartition.entrySet().stream().map(partitionWithFileList -> waitForCondition(partitionWithFileList.getKey(),
        partitionWithFileList.getValue().stream(), visibility)).allMatch(x -> x);
   ...
   */
  public abstract HoodieCleanerPlan scheduleClean();
   ...
   */
  public abstract List<HoodieCleanStat> clean(HoodieInstant cleanInstant,
   ...
   */
  public abstract List<HoodieRollbackStat> rollback(HoodieInstant instant, boolean deleteInstants)
      throws IOException;
