...
package org.apache.hudi.writer.roolback;
...
public class RollbackHelper implements Serializable {
   ...
   */
  public List<HoodieRollbackStat> performRollback(HoodieInstant instantToRollback, List<RollbackRequest> rollbackRequests) {
    ...
    Map<String, List<Pair<String, HoodieRollbackStat>>> partitionPathToRollbackStat = rollbackRequests.stream().map(rollbackRequest -> {
      ...
      switch (rollbackRequest.getRollbackAction()) {
        case DELETE_DATA_FILES_ONLY: {
          try {
            ...
            LOG.error("deleteCleanedFiles failed ", e);
          }
          return Pair.of(rollbackRequest.getPartitionPath(),
        ...
        case APPEND_ROLLBACK_BLOCK: {
          ...
          try {
            ...
            LOG.error("filesToNumBlocksRollback failed ", e);
          }
          return Pair.of(rollbackRequest.getPartitionPath(),
              HoodieRollbackStat.newBuilder().withPartitionPath(rollbackRequest.getPartitionPath())
    ...
    List<HoodieRollbackStat> result = new ArrayList<>();
    for (Map.Entry<String, List<Pair<String, HoodieRollbackStat>>> entry : partitionPathToRollbackStat.entrySet()) {
      Optional<HoodieRollbackStat> reduce = entry.getValue().stream().map(Pair::getRight).reduce(this::mergeRollbackStat);
      reduce.ifPresent(result::add);
    }
    return result;
  }
