...
public class HoodieFlinkMergeOnReadTable<T extends HoodieRecordPayload> extends HoodieFlinkCopyOnWriteTable<T> {
  ...
  HoodieFlinkMergeOnReadTable(HoodieWriteConfig config, Configuration hadoopConf, HoodieTableMetaClient metaClient) {
  ...
  public HoodieWriteOutput<List<WriteStatus>> compact(Configuration jsc, String compactionInstantTime,
    ...
    HoodieMergeOnReadTableCompactor compactor = new HoodieMergeOnReadTableCompactor();
    try {
      return new HoodieWriteOutput<>(compactor.compact(jsc, compactionPlan, this, config, compactionInstantTime));
    } catch (IOException e) {
