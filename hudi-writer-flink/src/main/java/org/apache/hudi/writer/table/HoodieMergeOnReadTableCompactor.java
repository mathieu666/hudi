...
import org.apache.hudi.writer.table.strategy.CompactionStrategy;
...
public class HoodieMergeOnReadTableCompactor implements HoodieCompactor {
  ...
  public HoodieMergeOnReadTableCompactor(HoodieTable table) {
    this.table = table;
  }

  @Override
  public List<WriteStatus> compact(HoodieCompactionPlan compactionPlan,
                                   HoodieTable hoodieTable, HoodieWriteConfig config, String compactionInstantTime) throws IOException {
    if (compactionPlan == null || (compactionPlan.getOperations() == null)
        || (compactionPlan.getOperations().isEmpty())) {
      return (List<WriteStatus>) Collections.EMPTY_LIST;
    }
    ...
    return operations.stream().flatMap(s -> compact(table, metaClient, config, s, compactionInstantTime)).collect(toList());
  }

  private List<WriteStatus> compact(HoodieTable table, HoodieTableMetaClient metaClient,
                                    HoodieWriteConfig config, CompactionOperation operation, String instantTime) {
    ...
    if (oldDataFileOpt.isPresent()) {
      result = table.handleUpdate(instantTime, operation.getPartitionPath(),
              operation.getFileId(), scanner.getRecords(),
          oldDataFileOpt.get());
    } else {
      result = table.handleInsert(instantTime, operation.getPartitionPath(), operation.getFileId(),
          scanner.iterator());
    }
  ...
  @Override
  public HoodieCompactionPlan generateCompactionPlan(HoodieTable hoodieTable,
