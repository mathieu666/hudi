...
import org.apache.hudi.writer.config.HoodieHBaseIndexConfig;
...
public class HBaseIndex<T extends HoodieRecordPayload> extends HoodieIndex<T> {
  ...
  @Override
  public HoodieWriteKey<List<HoodieKey>> fetchRecordLocation(HoodieWriteKey<List<HoodieKey>> hoodieKeys,
                                             Configuration hadoopConf, HoodieTable<T> hoodieTable) {
  ...
  @Override
  public HoodieWriteInput<List<HoodieRecord<T>>> tagLocation(HoodieWriteInput<List<HoodieRecord<T>>> recordRDD, Configuration hadoopConf,
    ...
    try (HTable hTable = (HTable) hbaseConnection.getTable(TableName.valueOf(tableName))) {
      ...
      // Do the tagging.
      Iterator<HoodieRecord<T>> hoodieRecordIterator = recordRDD.getInputs().iterator();
  ...
  @Override
  public HoodieWriteOutput<List<WriteStatus>> updateLocation(HoodieWriteOutput<List<WriteStatus>> writeStatusRDD, Configuration hadoopConf,
                                             HoodieTable<T> hoodieTable) {
    ...
    synchronized (HBaseIndex.class) {
      ...
      Iterator<WriteStatus> statusIterator = writeStatusRDD.getOutput().iterator();
