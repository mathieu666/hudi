...
public class HBaseIndex<T extends HoodieRecordPayload> extends HoodieIndex<T> {
  ...
  @Override
  public List<HoodieKey> fetchRecordLocation(List<HoodieKey> hoodieKeys,
  ...
  @Override
  public List<HoodieRecord<T>> tagLocation(List<HoodieRecord<T>> recordRDD, Configuration hadoopConf,
  ...
  @Override
  public List<WriteStatus> updateLocation(List<WriteStatus> writeStatusRDD, Configuration hadoopConf,
                                             HoodieTable<T> hoodieTable) {
    ...
    synchronized (HBaseIndex.class) {
      ...
      Iterator<WriteStatus> statusIterator = writeStatusRDD.iterator();
  ...
  public Tuple2<Long, Integer> getHBasePutAccessParallelism(final List<WriteStatus> writeStatusRDD) {
    final List<Tuple2<Long, Integer>> insertOnlyWriteStatusRDD = writeStatusRDD.stream()
