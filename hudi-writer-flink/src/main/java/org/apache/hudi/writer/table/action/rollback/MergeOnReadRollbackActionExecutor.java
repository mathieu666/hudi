...
package org.apache.hudi.writer.table.action.rollback;
...
public class MergeOnReadRollbackActionExecutor extends BaseRollbackActionExecutor {
  ...
  public MergeOnReadRollbackActionExecutor(Configuration hadoopConf,
                                           HoodieWriteConfig config,
                                           HoodieTable<?> table,
                                           String instantTime,
                                           HoodieInstant commitInstant,
                                           boolean deleteInstants) {
    super(hadoopConf, config, table, instantTime, commitInstant, deleteInstants);
  }
  ...
  public MergeOnReadRollbackActionExecutor(Configuration hadoopConf,
    ...
    super(hadoopConf, config, table, instantTime, commitInstant, deleteInstants, skipTimelinePublish);
  }
  ...
  protected List<HoodieRollbackStat> executeRollback() throws IOException {
    ...
    if (!resolvedInstant.isRequested()) {
      ...
      List<RollbackRequest> rollbackRequests = generateRollbackRequests(hadoopConf, resolvedInstant);
      // TODO: We need to persist this as rollback workload and use it in case of partial failures
      allRollbackStats = new RollbackHelper(table.getMetaClient(), config).performRollback(hadoopConf, resolvedInstant, rollbackRequests);
    }
   ...
   * @throws IOException
   */
  private List<RollbackRequest> generateRollbackRequests(Configuration jsc, HoodieInstant instantToRollback)
      throws IOException {
