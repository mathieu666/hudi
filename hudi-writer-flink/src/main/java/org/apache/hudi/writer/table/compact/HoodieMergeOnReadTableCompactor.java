...
package org.apache.hudi.writer.table.compact;
...
import org.apache.hudi.writer.table.HoodieCopyOnWriteTable;
...
//  private AccumulatorV2<Long, Long> totalFileSlices;
  ...
  public List<WriteStatus> compact(Configuration hadoopConf, HoodieCompactionPlan compactionPlan,
    ...
    HoodieCopyOnWriteTable table = new HoodieCopyOnWriteTable(config, hadoopConf, metaClient);
    List<CompactionOperation> operations = compactionPlan.getOperations().stream()
        .map(CompactionOperation::convertFromAvroRecordInstance).collect(toList());
    LOG.info("Compactor compacting " + operations + " files");

    return operations.stream().map(x -> {
      List<WriteStatus> compact = new ArrayList<>();
      try {
  ...
  private List<WriteStatus> compact(HoodieCopyOnWriteTable hoodieCopyOnWriteTable, HoodieTableMetaClient metaClient,
    ...
    List<String> logFiles = operation.getDeltaFileNames().stream().map(
        ...
        .collect(toList());
    long maxMemoryPerCompaction = 512;
  ...
  @Override
  public HoodieCompactionPlan generateCompactionPlan(Configuration hadoopConf, HoodieTable hoodieTable,
