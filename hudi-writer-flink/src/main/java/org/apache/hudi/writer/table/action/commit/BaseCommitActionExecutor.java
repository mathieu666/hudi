...
public abstract class BaseCommitActionExecutor<T extends HoodieRecordPayload<T>>
  ...
  private Map<Integer, List<HoodieRecord<T>>> partition(List<HoodieRecord<T>> dedupedRecords, Partitioner partitioner) {
    Map<Integer, List<Tuple2<Tuple2<HoodieKey, Option<HoodieRecordLocation>>, HoodieRecord<T>>>> partitionedMidRecords = dedupedRecords.stream().map(
  ...
  protected void updateIndexAndCommitIfNeeded(List<WriteStatus> writeStatusRDD, HoodieWriteMetadata result) {
    ...
    List<WriteStatus> statuses = ((HoodieTable<T>) table).getIndex().updateLocation(writeStatusRDD, hadoopConf,
        (HoodieTable<T>) table);
...
//    commitOnAutoCommit(result);
  ...
  private void commit(Option<Map<String, String>> extraMetadata, HoodieWriteMetadata result) {
    ...
    List<HoodieWriteStat> stats = result.getWriteStatuses().stream().map(WriteStatus::getStat).collect(Collectors.toList());
