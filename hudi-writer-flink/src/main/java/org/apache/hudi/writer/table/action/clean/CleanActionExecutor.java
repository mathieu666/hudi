...
package org.apache.hudi.writer.table.action.clean;
...
public class CleanActionExecutor extends BaseActionExecutor<HoodieCleanMetadata> {
   ...
   * @return Cleaner Plan
   */
  HoodieCleanerPlan requestClean(Configuration jsc) {
    try {
      ...
      Map<String, List<String>> cleanOps = partitionsToClean
          .stream()
  ...
  private static Stream<Pair<String, PartitionCleanStat>> deleteFilesFunc(Iterator<Tuple2<String, String>> in, HoodieTable table) {
    Map<String, PartitionCleanStat> partitionCleanStatMap = new HashMap<>();
    FileSystem fs = table.getMetaClient().getFs();
    Path basePath = new Path(table.getMetaClient().getBasePath());
    while (in.hasNext()) {
      Tuple2<String, String> partitionDelFileTuple = in.next();
   ...
   */
  List<HoodieCleanStat> clean(Configuration jsc, HoodieCleanerPlan cleanerPlan) {
    Iterator<Tuple2<String, String>> filesToBeDeletedPerPartition = cleanerPlan.getFilesToBeDeletedPerPartition().entrySet().stream()
        .flatMap(x -> x.getValue().stream().map(y -> new Tuple2<>(x.getKey(), y))).iterator();

    Stream<Tuple2<String, PartitionCleanStat>> partitionCleanStats =
        deleteFilesFunc(filesToBeDeletedPerPartition, table)
            .collect(Collectors.groupingBy(Pair::getLeft))
            .entrySet().stream()
            .map(x -> new Tuple2(x.getKey(), x.getValue().stream().map(y -> y.getRight()).reduce(PartitionCleanStat::merge)));
  ...
  Option<HoodieCleanerPlan> requestClean(String startCleanTime) {
    final HoodieCleanerPlan cleanerPlan = requestClean(hadoopConf);
  ...
  private HoodieCleanMetadata runClean(HoodieTable<?> table, HoodieInstant cleanInstant, HoodieCleanerPlan cleanerPlan) {
    ...
    try {
      ...
      List<HoodieCleanStat> cleanStats = clean(hadoopConf, cleanerPlan);
